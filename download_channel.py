...
quota = int(input("Enter max download amount in MB: ")) * 1000000 # Convert to bytes
...
while claims_left:
  result = requests.post(server, json={
    ...
    "params":
    {
      ...
      "creation_height": "<=%i" % last_height, # This allows to query >1000 claims (May cause some dublicates, but shouldn't matter)
      "page": page
    }
  }).json()["result"]
...
for i in claims:
  print(i.get("url"))
print("Total(Large channels may have some dublicates, these will be downloaded only ones): %s\n" % (len(claims)))
print("Known data on channel: %d / %d MB: " % (data_to_download/1000000, total_size/1000000))
...
def isDownloaded(i):
  try: 
    ...
    if blobs_completed == blobs_in_stream:
      print("Finished download for claim no %i: %s" % (i, claims[i]["url"]))
      return True
  except IndexError:
    pass
  return False
